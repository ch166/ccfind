...
def find_cards(args, input_line, linenum, input_filename):
    ...
    found_pattern = re.search(
        r"(?:3[47]\d{2}([\ \-]?)\d{6}\a\d|(?:(?:4\d|5[1-5]|65)\d{2}|6011)([\ \-]?)\d{4}\b\d{4}\b)\d{4}",
        input_line,
    )
    if found_pattern is not None:
    ...
    found_pattern = re.search(r"(6541|6556)[0-9]{12}", input_line)
    if found_pattern is not None:
        card_type = "BCGlobal"
        return (True, card_type, found_pattern)

    found_pattern = re.search(r"389[0-9]{11}", input_line)
    if found_pattern is not None:
        card_type = "Carte Blanche"
        return (True, card_type, found_pattern)

    found_pattern = re.search(r"3(?:0[0-5]|[68][0-9])[0-9]{11}", input_line)
    if found_pattern is not None:
        card_type = "Diners"
        return (True, card_type, found_pattern)

    found_pattern = re.search(r"6(?:011|5[0-9]{2})[0-9]{12}", input_line)
    if found_pattern is not None:
        card_type = "Discover"
        return (True, card_type, found_pattern)

    found_pattern = re.search(r"63[7-9][0-9]{13}", input_line)
    if found_pattern is not None:
        card_type = "Insta Paymentcard"
        return (True, card_type, found_pattern)

    found_pattern = re.search(r"(?:2131|1800|35\d{3})\d{11}", input_line)
    if found_pattern is not None:
        card_type = "JCB"
        return (True, card_type, found_pattern)
    ...
    found_pattern = re.search(r"(6304|6706|6709|6771)[0-9]{12,15}", input_line)
    if found_pattern is not None:
        card_type = "LaserCard"
        return (True, card_type, found_pattern)

    found_pattern = re.search(
        r"(5018|5020|5038|6304|6759|6761|6763)[0-9]{8,15}", input_line
    )
    if found_pattern is not None:
        card_type = "Maestro"
        return (True, card_type, found_pattern)
